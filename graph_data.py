...
mal_cs_data = mal.filter(regex='grp_')
...
ben_cs_data = ben.filter(regex='grp_')
...
cs_sum = pd.DataFrame({"Benign": ben_cs_data.sum(), "Malware": mal_cs_data.sum()}, index=ben_cs_data.sum().sort_values().index)
#cs_sum = cs_sum.loc[:, (cs_sum != 0).any(axis=0)]
...
#cs_sum = pd.concat([ben_cs_data, mal_cs_data], axis=1).sort_values(by='Malware').tail(40)
#mal_cs_nz = len(mal_cs_data.loc[:, (mal_cs_data != 0).any(axis=0)].columns)
#ben_cs_nz = len(ben_cs_data.loc[:, (ben_cs_data != 0).any(axis=0)].columns)
